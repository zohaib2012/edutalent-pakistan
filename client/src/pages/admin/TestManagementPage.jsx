...
import { Plus, Upload, Edit, Trash2, Search, Eye, X, ShieldAlert, Clock, Loader2 } from 'lucide-react';
...
export default function TestManagementPage() {
  ...
  const [form, setForm] = useState({
    ...
    correctAnswer: '', subject: '', phase: '', difficulty: 'Medium', timeLimit: '60',
  });
  ...
  const handleSaveQ = async () => {
    if (!form.question.trim() || !form.correctAnswer) return;
    try {
      await api.post('/questions', {
        questionText: form.question, options: [
          { label: 'A', text: form.optionA }, { label: 'B', text: form.optionB },
          { label: 'C', text: form.optionC }, { label: 'D', text: form.optionD },
        ],
        correctAnswer: form.correctAnswer, subject: form.subject, difficulty: form.difficulty,
        phaseId: form.phase, timeLimit: Number(form.timeLimit),
      });
      setShowAddModal(false);
      setForm({ question: '', optionA: '', optionB: '', optionC: '', optionD: '', correctAnswer: '', subject: '', phase: '', difficulty: 'Medium', timeLimit: '60' });
      fetchQuestions();
    } catch (err) {
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {loading ? (
                        ...
                        <tr><td colSpan={5} className="text-center py-10 text-gray-400">No questions found.</td></tr>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
              <div className="bg-white rounded-xl w-full max-w-2xl mx-4 shadow-2xl">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="text" value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })}
                        placeholder="Subject" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[#1A73E8]" />
                    </div>
                    <div>
                ...
                <div className="flex items-center justify-end gap-3 p-5 border-t border-gray-100">
                  ...
                  <button onClick={handleSaveQ} className="bg-[#1A73E8] text-white px-5 py-2.5 rounded-lg text-sm font-medium hover:bg-[#1557B0]">Save</button>
                </div>
              </div>
            </div>
          )}
